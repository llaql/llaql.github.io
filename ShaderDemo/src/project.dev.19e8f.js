window.__require = function e(t, n, r) {
  ...
  InputePropertys: [ function(require, module, exports) {
    ...
    var InputePropertys = function(_super) {
      ...
      InputePropertys.prototype.setSliderActions = function(titleName, paramName, defaultValues, actions) {
        var _this = this;
        this.title.string = titleName;
        this.ips.forEach(function(ip) {
          ip.node.active = false;
        ...
        actions.forEach(function(action, index) {
          _this.ips[index].node.active = true;
          _this.ips[index].setEditorAction(paramName, defaultValues[index], action);
          _this.ips[index].setSliderAction(paramName, defaultValues[index], action);
        });
      };
  ...
  InputeProperty: [ function(require, module, exports) {
    ...
    var InputeProperty = function(_super) {
      ...
      InputeProperty.prototype.setSliderAction = function(paramName, defaultValues, action) {
        this.paramName = paramName;
        this.sliderAction = action;
        this.editorAction = action;
        this.slider.progress = defaultValues / 255;
        var event = new cc.Component.EventHandler();
        event.target = this.node;
        event.component = "InputeProperty";
      ...
      InputeProperty.prototype.doSliderAction = function() {
        var value = Math.floor(255 * this.slider.progress);
        ...
        switch (param.type) {
         case ShaderFunc_1.RENDERER_PARAMS.PARAM_FLOAT:
          if (isColor) {
            ...
            inputePropertys.setSliderActions(param.name + "\ud83c\udf08", param.name, [ Math.floor(255 * d) ], [ genPropertyAction(m, param.type, 0, param.paramType) ]);
          } else {
         ...
         case ShaderFunc_1.RENDERER_PARAMS.PARAM_FLOAT3:
          if (isColor) {
            ...
            propertyPrefab.setParent(this.propertyParent);
            inputePropertys = propertyPrefab.getComponent(InputePropertys_1.default);
            inputePropertys.setSliderActions(param.name + "\ud83c\udf08", param.name, [ isColor ? Math.floor(255 * d.x) : d.x, isColor ? Math.floor(255 * d.y) : d.y, isColor ? Math.floor(255 * d.z) : d.z ], [ genPropertyAction(m, param.type, 0, param.paramType), genPropertyAction(m, param.type, 1, param.paramType), genPropertyAction(m, param.type, 2, param.paramType) ]);
          } else {
            propertyPrefab = cc.instantiate(this.params[0]);
            propertyPrefab.setParent(this.propertyParent);
            inputePropertys = propertyPrefab.getComponent(InputePropertys_1.default);
